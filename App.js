...
// === PredictHQ API token (updated) ===
const API_TOKEN = "Um313WT-nPl-BwYZ8sCo-yaPr-RW1IsouR9yo5dr";
...
function normalizeCountryName(name = "") {
  ...
  const m = {
    ...
    "Korea (Republic of)": "South Korea",
    "Viet Nam": "Vietnam",
    "UAE": "United Arab Emirates", // not in list, but kept in case
  };
  return m[n] || n;
}

function getContinentFromCountry(countryName) {
  for (const [cont, arr] of Object.entries(countries)) {
    if (arr.includes(countryName)) return cont;
  }
  return null;
}

function pickCity(countryName, candidateNames = []) {
  const list = cities[countryName] || [];
  if (!list.length) return null;

  // Exact match first
  for (const cand of candidateNames) {
    const hit = list.find((c) => c.toLowerCase() === String(cand || "").toLowerCase());
    if (hit) return hit;
  }
  // Loose contains match
  for (const cand of candidateNames) {
    const l = String(cand || "").toLowerCase();
    const hit = list.find((c) => c.toLowerCase().includes(l) || l.includes(c.toLowerCase()));
    if (hit) return hit;
  }
  // Fallback to largest city in list
  return list[0];
}
...
export default function App() {
  ...
  const getPreciseLocation = useCallback(async () => {
    try {
      ...
      if (!servicesOn) {
        Alert.alert(
          "Enable Location",
          "Please turn on Location Services (GPS) for precise results."
        );
      }
      ...
      if (status !== "granted") {
        if (canAskAgain) {
          Alert.alert("Permission Required", "Please grant location permission for accurate results.");
        }
        return null;
      }

      // Quick last known (may be stale) to show something, then precise lock
      const last = await Location.getLastKnownPositionAsync();
      let current = null;

      // Highest accuracy GPS lock with a sensible timeout
      // BestForNavigation => finest GPS on both iOS/Android
      ...
      // Prefer precise; fallback to last known if needed
      const chosen = current || last;
      ...
      // Reverse geocode -> normalized country/city pick
      const geos = await Location.reverseGeocodeAsync(chosen.coords);
      if (geos && geos.length) {
        const g = geos[0];
        const normalizedCountry = normalizeCountryName(g.country || "");
        const continentGuess = getContinentFromCountry(normalizedCountry);
        ...
        setCity(pickCity(normalizedCountry, candidateNames));
        setTimeRange((prev) => prev || "thisyear");
      }

      return chosen.coords;
    } catch (err) {
      console.warn("Precise location error:", err?.message || err);
      Alert.alert(
        "Location Error",
        "Couldn't get a precise GPS lock. You can still pick location manually."
      );
      return null;
    }
  }, []);

  // Detect location on mount (razor-sharp attempt)
  useEffect(() => {
    (async () => {
  ...
  }, [continent, country, city, getPreciseLocation]);

  // Auto-search for events after autofill (first time only)
  useEffect(() => {
    if (continent && country && city && timeRange && !firstAutoSearchDone) {
      fetchEvents();
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      params.set("start.gte", start.toISOString());
      params.set("start.lte", end.toISOString());
      params.set("sort", "start"); // earliest first
      params.set("limit", "50");

      // If we have precise coordinates, use a geospatial query (much more accurate than "q=city")
      if (coords?.latitude && coords?.longitude) {
        // 50km radius around precise location (tweakable)
        params.set("within", `50km@${coords.latitude},${coords.longitude}`);
      } else if (city) {
        // Fallback to text query if we don't have coords
        params.set("q", city);
      }
      ...
      if (!resp.ok) throw new Error(`PredictHQ HTTP ${resp.status}`);
      ...
      setEvents([]);
      Alert.alert("Events Error", "Couldn't load events right now. Try again shortly.");
    } finally {
      setLoading(false);
    }
  }, [timeRange, coords, city]);

  useEffect(() => {
    if (timeRange && (coords || city)) {
      fetchEvents();
    }
  }, [timeRange, coords, city, fetchEvents]);
              ...
              <View style={styles.quickRow}>
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    if (took > 0) {
                      // Optional toast substitute
                      console.log(`Precision refresh took ${took}ms`);
                    }
                  }}
                >
                  <Icon name="crosshairs-gps" size={18} color="#fff" />
                  <Text style={styles.precisionBtnText}>
                    Use Precise Location{coords?.accuracy ? ` (~${Math.round(coords.accuracy)}m)` : ""}
                  </Text>
                </TouchableOpacity>

                {city && (
                  <View style={styles.locationChip}>
                    <Icon name="map-marker" size={16} color="#302b63" />
            ...
            <View key={index} style={styles.card}>
              ...
              {item.entities && item.entities[0] && (
                ...
                >
                  ...
                  <Text style={[styles.linkText, { color: "#1e88e5", textDecorationLine: "underline" }]}>
                    {item.entities[0].name}
                  </Text>
                </TouchableOpacity>
              )}
              {item.url && (
                <TouchableOpacity style={styles.cardLink} onPress={() => Linking.openURL(item.url)} activeOpacity={0.7}>
